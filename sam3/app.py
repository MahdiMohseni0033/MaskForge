"""Streamlit UI for official SAM 3 rectangle-prompt segmentation."""
...
from sam3_app import ModelConfigurationError, SAM3Paths, load_sam3_model, run_inference
from sam3_app.canvas_prompts import boxes_from_canvas
from sam3_app.inference import BoxPrompt
from sam3_app.preprocessing import ImageValidationError, ensure_rgb
from sam3_app.streamlit_compat import st_canvas
from sam3_app.visualization import make_overlay, mask_to_image


st.set_page_config(page_title="SAM 3 prompt segmentation", page_icon="✂️", layout="wide")
st.title("SAM 3 prompt segmentation")
st.caption("Draw one or more areas to guide the official SAM 3 model.")
st.warning("Technical research prototype only — not a clinically validated medical device.", icon="⚠️")


@st.cache_resource(show_spinner="Loading the official SAM 3 model…")
def cached_model(paths: SAM3Paths, device: str):
    return load_sam3_model(paths, device=device)  # type: ignore[arg-type]
...
with st.sidebar:
    ...
    st.divider()
    st.caption("The image is encoded once at SAM 3's native resolution and reused for every area.")
...
if st.button("2. Run SAM 3 segmentation", type="primary"):
    paths = SAM3Paths.from_environment()
    try:
        loaded = cached_model(paths, device)
        with st.spinner(f"Encoding the image and decoding {len(box_prompts)} selected area(s)…"):
            result = run_inference(loaded, image, box_prompts)
    ...
    st.image(overlay, caption="SAM 3 area-prompt overlay", width="stretch")
    message = f"Returned {result.instance_count} mask(s) from {len(box_prompts)} selected area(s)."
    if result.mean_quality is not None:
        message += f" Mean predicted quality: {result.mean_quality:.3f}."
    st.success(message)
    downloads = st.columns(2)
    with downloads[0]:
        st.download_button("Download mask PNG", image_png_bytes(mask_image), "sam3_mask.png", "image/png")
    with downloads[1]:
        st.download_button("Download overlay PNG", image_png_bytes(overlay), "sam3_overlay.png", "image/png")
